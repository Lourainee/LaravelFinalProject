...
<?php $__env->startSection('title', 'Tasks - TaskFlow'); ?>
<?php $__env->startSection('page-title', 'Tasks'); ?>

<?php $__env->startSection('content'); ?>
<div class="mb-6 flex flex-col md:flex-row gap-4">
    <a href="<?php echo e(route('tasks.create')); ?>" class="px-4 py-2.5 bg-white/5 text-slate-100 rounded-2xl hover:bg-white/10 transition font-semibold ring-1 ring-white/10">
        Create Task
    </a>
    
    <!-- Search and Filter -->
    <form method="GET" action="<?php echo e(route('tasks.index')); ?>" class="flex gap-2 flex-1">
        <input type="text" name="search" placeholder="Search tasks..." value="<?php echo e(request('search')); ?>"
                ...
                <tbody>
                    <?php $__currentLoopData = $tasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="border-b border-white/10 hover:bg-white/5 transition">
                            <td class="px-6 py-4">
                                <a href="<?php echo e(route('tasks.edit', $task)); ?>" class="font-semibold text-sky-300 hover:text-white underline-offset-4 hover:underline">
                                    <?php echo e($task->title); ?>

                                </a>
                            </td>
                            <td class="px-6 py-4">
                                <?php if($task->categories->count() > 0): ?>
                                    <div class="flex flex-wrap gap-1">
                                        <?php $__currentLoopData = $task->categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <span class="px-2 py-1 text-xs text-white rounded-lg ring-1 ring-white/10" data-bg="<?php echo e($cat->color); ?>">
                                                <?php echo e($cat->name); ?>
                            ...
                            <td class="px-6 py-4">
                                <?php if($task->due_date): ?>
                                    <span class="<?php echo e($task->isOverdue() ? 'text-rose-300 font-semibold' : 'text-slate-300'); ?>">
                            ...
                            <td class="px-6 py-4">
                                <form action="<?php echo e(route('tasks.toggle-status', $task)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>
                            ...
                            <td class="px-6 py-4 space-x-2">
                                <a href="<?php echo e(route('tasks.edit', $task)); ?>" class="text-slate-200 hover:text-white font-semibold text-sm">Edit</a>
                                <form action="<?php echo e(route('tasks.destroy', $task)); ?>" method="POST" class="inline" onsubmit="return confirm('Are you sure?');">
                                    <?php echo csrf_field(); ?>
...
<?php else: ?>
    <div class="glass rounded-3xl shadow-2xl shadow-black/30 ring-1 ring-white/10 p-12 text-center">
        ...
        <a href="<?php echo e(route('tasks.create')); ?>" class="inline-block px-6 py-2.5 bg-gradient-to-r from-indigo-500 to-sky-400 text-slate-950 rounded-2xl hover:opacity-95 transition font-semibold shadow-lg shadow-indigo-500/20">
            Create Task
        </a>
    </div>
<?php endif; ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\pc\LaravelFinalProject\resources\views/tasks/index.blade.php ENDPATH**/ ?>