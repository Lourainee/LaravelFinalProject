            ...
            <div class="w-72 glass shadow-2xl shadow-black/40 flex flex-col">
                ...
                <nav class="flex-1 p-4 space-y-2">
                    <a href="{{ route('dashboard') }}" 
                        ...
                        {{ request()->routeIs('dashboard') ? 'bg-white/10 text-white ring-1 ring-white/10' : 'hover:bg-white/5 text-slate-300' }}">
                        <span class="h-2 w-2 rounded-full {{ request()->routeIs('dashboard') ? 'bg-sky-400' : 'bg-slate-600' }}"></span>
                        <span class="font-medium">Dashboard</span>
                    </a>
                    <a href="{{ route('tasks.index') }}" 
                        class="block px-4 py-3 rounded-xl transition flex items-center gap-3
                        {{ request()->routeIs('tasks.*') ? 'bg-white/10 text-white ring-1 ring-white/10' : 'hover:bg-white/5 text-slate-300' }}">
                        <span class="h-2 w-2 rounded-full {{ request()->routeIs('tasks.*') ? 'bg-indigo-400' : 'bg-slate-600' }}"></span>
                        <span class="font-medium">Tasks</span>
                    </a>
                    <a href="{{ route('categories.index') }}" 
                        class="block px-4 py-3 rounded-xl transition flex items-center gap-3
                        {{ request()->routeIs('categories.*') ? 'bg-white/10 text-white ring-1 ring-white/10' : 'hover:bg-white/5 text-slate-300' }}">
                        <span class="h-2 w-2 rounded-full {{ request()->routeIs('categories.*') ? 'bg-emerald-400' : 'bg-slate-600' }}"></span>
                ...
                <div class="px-6 pt-4">
                    @if(session('success'))
                        <div class="mb-4 p-4 glass rounded-2xl flex items-center">
                            ...
                            <span class="text-emerald-100">{{ session('success') }}</span>
                        </div>
                    @endif
